...
def locate_gears(engine_schematic: list[str]) -> list[Pos]:
    positions = []
...
def main():
    ...
    gear_positions = locate_gears(engine_schematic)
    sum_ = 0

    for position in gear_positions:
        adj_nums = get_adjacent_numbers(engine_schematic, position)

        if len(adj_nums) == 2:
            sum_ += math.prod(adj_nums)
