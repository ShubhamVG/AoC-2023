...
int gmap_contains(GMap gmap, long long int val);
int is_null_gmap(GMap gmap);
long long int xval_from_yval(GMap x[], long long int yval);
...
void init() {
    ...
    GMap null_gmap = (GMap) {.dest_start = -1, .source_start = -1, .range = -1};
...
int main(int argc, char* argv[]) {
    ...
    long long int min_location = 999999999999999;
    ...
    for (int i = 0; i < MAX_SEEDS; i++) {
        ...
        long long int soil = xval_from_yval(seed_to_soil, seeds[i]);
        long long int fertilizer = xval_from_yval(soil_to_fertilizer, soil);
        long long int water = xval_from_yval(fertilizer_to_water, fertilizer);
        long long int light = xval_from_yval(water_to_light, water);
        long long int temp = xval_from_yval(light_to_temp, light);
        long long int humidity = xval_from_yval(temp_to_humidity, temp);
        long long int location = xval_from_yval(humidity_to_location, humidity);
...
long long int xval_from_yval(GMap x[], long long int yval) {
    long long int xval = yval; // default
    ...
    for (int i = 0; i < MAX_ARRAY_LEN; i++) {
        ...
        if (gmap_contains(gmap, yval) == 1) {
            long long int difference = yval - gmap.source_start;
